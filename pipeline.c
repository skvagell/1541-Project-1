...
int main(int argc, char **argv)
{
  ...
  while(1) {
    ...
    size = get_next_instr(tr_entry);
    if (!size) {       /* no more instructions (trace_items) to simulate */
      if(is_empty(buffers)){ /*the pipeline is empty*/
        printf("+ Simulation terminates at cycle : %u\n", cycle_number);
        break;
      }
      buffers[IF2] = NO_OP;/*no next instruction but pipeline isn't empty yet. pump a NO_OP into pipeline*/
    }
